...
### 改进
- **新增资质工单查询：自动选择审核状态为"审核通过"**
  - 在输入PID后、点击查询之前，自动选择审核状态为"审核通过"
  - 过滤掉未审核通过或审核不通过的工单，减少需要处理的工单数量
  - 提高查询效率：只处理审核通过的工单，避免处理过多无关工单
  - 如果当前已选择"审核通过"，则跳过设置步骤
  - 如果设置失败，会继续使用默认设置，不影响查询流程
- **优化资质工单查询：添加查询按钮点击重试逻辑**
  - 新增 `click_query_button_with_retry()` 函数，支持自动重试机制
  - 默认重试3次，每次重试前等待2秒
  - 提高查询稳定性：网络波动或页面加载慢时自动重试
  - 在所有查询按钮点击位置统一使用重试逻辑
  - 如果重试失败，会返回明确的错误信息
- **优化资质工单查询：自动设置每页显示100条**
  - 修正设置时机：在输入PID并查询后、查找包含'短信资质'的行之前设置分页器为"100 条/页"
  - 提高查询效率：每页显示更多数据，减少翻页操作
  - 如果设置失败，会继续使用默认设置，不影响查询流程
- **重构资质工单检查逻辑：统一使用工单号精确查询**
  - 修改检查流程：对每个工单号，统一进入查询页面 → 输入工单号 → 点击查询 → 进入详情页面
  - 简化逻辑：移除了复杂的PID查询和分页查找逻辑
  - 提高效率：精确查询工单号后，通常只有一个结果，直接点击进入详情页面
  - 统一处理：所有工单都使用相同的查询方式，代码更简洁易维护
  - 移除冗余：不再需要判断是否是第一个工单，所有工单都使用相同的处理流程
