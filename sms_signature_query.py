...
if __name__ == '__main__':
    ...
    async def main():
        ...
        try:
            ...
            if signature_result.get('success') and signature_result.get('work_order_id'):
                print("\n" + "="*60)
                print("开始查询资质工单...")
                print("="*60)
                ...
                if pid:
                    qualification_result = await query_qualification_work_order(
                        page=page,
                        work_order_id=signature_result['work_order_id'],
                        pid=pid
                    )
                    
                    if qualification_result.get('success'):
                        print(f"\n[OK] 资质工单查询成功！")
                        print(f"匹配的资质工单号: {qualification_result['work_order_id']}")
                        print(f"关联资质ID: {qualification_result['qualification_id']}")
                        print(f"资质组ID: {qualification_result['qualification_group_id']}")
                    else:
                        print(f"\n[FAIL] 资质工单查询失败: {qualification_result.get('error', '未知错误')}")
                else:
                    print("\n[WARN] 未配置PID，跳过资质工单查询")
                    qualification_result = {'success': False, 'error': '未配置PID'}
            else:
                print("\n[WARN] 工单号查询失败，跳过资质工单查询")
                qualification_result = {'success': False, 'error': '工单号查询失败'}
            ...
            if qualification_result:
                print(f"\n资质工单号信息：")
                if qualification_result.get('success'):
                    print(f"  匹配的资质工单号: {qualification_result.get('work_order_id', 'N/A')}")
                    print(f"  关联资质ID: {qualification_result.get('qualification_id', 'N/A')}")
                    print(f"  资质组ID: {qualification_result.get('qualification_group_id', 'N/A')}")
                else:
                    print(f"  查询失败: {qualification_result.get('error', '未知错误')}")
            else:
                print(f"\n资质工单号信息：未执行查询")
